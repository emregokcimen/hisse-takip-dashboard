...
function computeSignal(input = {}) {
  ...
  const latestPrice = numberOrNull(input.price) ?? points.at(-1)?.close ?? null;
  if (!symbol || points.length < 30 || !Number.isFinite(latestPrice)) {
    return emptySignal(symbol, latestPrice, "insufficient_data", "Sinyal için yeterli geçmiş veri yok.");
  }

  const indicators = computeIndicators(points, latestPrice);
  const target = numberOrNull(input.fibTarget);
  const targetDistancePct = Number.isFinite(target) && target !== 0 ? ((target - latestPrice) / target) * 100 : null;
  const targetStatus = getTargetStatus(latestPrice, target, numberOrNull(input.alertThreshold) ?? 0.5);
  ...
  addRule(scoreParts, "Trend", indicators.sma50 > indicators.sma200, 16, -16, reasons, "MA50 MA200 üstünde", "MA50 MA200 altında");
  addRule(scoreParts, "Kısa trend", indicators.sma20 > indicators.sma50, 10, -8, reasons, "MA20 MA50 üstünde", "MA20 MA50 altında");
  addRule(scoreParts, "MACD", indicators.macdHistogram > 0, 12, -12, reasons, "MACD pozitif bölgede", "MACD negatif bölgede");
  addRule(scoreParts, "RSI", indicators.rsi14 < 35, 8, indicators.rsi14 > 72 ? -10 : 0, reasons, `RSI ${round(indicators.rsi14, 1)} aşırı satıma yakın`, `RSI ${round(indicators.rsi14, 1)} aşırı alıma yakın`);
  addRule(scoreParts, "52H", indicators.high52wDistancePct > -25, 8, -6, reasons, "Fiyat 52 hafta zirvesine makul yakın", "Fiyat 52 hafta zirvesinden uzak");
  addRule(scoreParts, "Momentum", indicators.momentum20d > 0, 12, -10, reasons, `20 günlük momentum ${round(indicators.momentum20d, 1)}%`, `20 günlük momentum ${round(indicators.momentum20d, 1)}%`);
  ...
  score += scoreParts.reduce((sum, item) => sum + item.value, 0);
  score = Math.max(0, Math.min(100, Math.round(50 + score)));
  ...
  const confidence = computeConfidence({ points, indicators, freshnessSec, reasons });
  const direction = signal === "strong_buy" || signal === "buy" ? "up" : signal === "sell" || signal === "risky" ? "down" : "flat";
...
function computeIndicators(points, latestPrice) {
  ...
  const macdLine = ema12Values.map((value, index) => Number.isFinite(value) && Number.isFinite(ema26Values[index]) ? value - ema26Values[index] : null);
  const macdSignalValues = emaSeries(macdLine.filter(Number.isFinite), 9);
  const macd = macdLine.filter(Number.isFinite).at(-1) ?? null;
  ...
  return {
    ...
    sma150,
    sma200,
    ema12: ema12Values.at(-1) ?? null,
    ema26: ema26Values.at(-1) ?? null,
    macd,
    macdSignal,
    ...
    bollingerBandwidthPct: Number.isFinite(bollinger.upper) && Number.isFinite(bollinger.lower) && Number.isFinite(bollinger.middle) && bollinger.middle !== 0
...
function evaluateAlarmRules(rules = [], context = {}) {
  return rules
...
function evaluateAlarmRule(rule = {}, context = {}) {
  ...
  const make = (message, severity = "info") => ({
    id: `${keyBase}:${type}`,
    ruleId: rule.id,
    symbol: row.symbol,
    type,
    title: rule.name || alertTypeLabel(type),
    message,
    severity,
    createdAt: Date.now()
  });

  if (type === "target_near" && Number.isFinite(row.targetDistancePct) && Math.abs(row.targetDistancePct) <= (threshold ?? 1)) {
    return make(`${row.symbol} Fib hedefe ${round(Math.abs(row.targetDistancePct), 2)}% uzaklıkta.`, "warning");
  }
  if (type === "fib_breakout" && row.targetStatus === "above") {
    return make(`${row.symbol} Fib hedef seviyesinin üstünde işlem görüyor.`, "success");
  }
  if (type === "rsi_extreme" && Number.isFinite(signal.indicators?.rsi14) && (signal.indicators.rsi14 <= (threshold ?? 30) || signal.indicators.rsi14 >= 70)) {
    return make(`${row.symbol} RSI ${round(signal.indicators.rsi14, 1)} seviyesinde.`, signal.indicators.rsi14 <= 30 ? "success" : "warning");
  }
  if (type === "macd_cross" && Number.isFinite(signal.indicators?.macdHistogram) && Math.abs(signal.indicators.macdHistogram) <= (threshold ?? 0.5)) {
    return make(`${row.symbol} MACD kesişim bölgesinde.`, "info");
  }
  if (type === "ma_trend_break" && Number.isFinite(signal.indicators?.sma50) && Number.isFinite(row.price) && row.price < signal.indicators.sma50) {
    return make(`${row.symbol} fiyatı MA50 altına indi.`, "danger");
  }
  if (type === "bollinger_breakout" && Number.isFinite(signal.indicators?.bollingerUpper) && Number.isFinite(row.price) && row.price > signal.indicators.bollingerUpper) {
    return make(`${row.symbol} Bollinger üst bandını kırdı.`, "success");
  }
  if (type === "volume_spike" && Number.isFinite(signal.indicators?.volumeSpikeRatio) && signal.indicators.volumeSpikeRatio >= (threshold ?? 1.8)) {
    return make(`${row.symbol} hacim ortalamanın ${round(signal.indicators.volumeSpikeRatio, 1)} katı.`, "warning");
  }
  if (type === "news_high" && row.newsImpact === "high") {
    return make(`${row.symbol} için yüksek etkili haber akışı var.`, row.newsSentiment === "negative" ? "danger" : "success");
  }
  if (type === "risk_rising" && (row.riskLevel === "high" || signal.risk === "high")) {
    return make(`${row.symbol} risk seviyesi yüksek.`, "danger");
  }
  return null;
}

function defaultAlarmRules() {
  return [
...
function matchesRuleScope(rule, row) {
  ...
  if (scope === "symbol") return String(rule.symbol || "").toUpperCase() === String(row.symbol || "").toUpperCase();
  if (scope === "category") return String(rule.category || "") === String(row.category || "");
  return true;
}
...
function addRule(parts, label, condition, positive, negative, reasons, positiveText, negativeText) {
  let value = 0;
  if (condition === true) {
    value = positive;
...
function emaSeries(values, period) {
  const input = values.map(numberOrNull);
...
function emptySignal(symbol, price, signal, reason) {
  return {
    symbol,
    price,
    signal,
